...
from sixa import BytesIO
from sixa.moves import http_client
from sixa.moves.urllib.parse import urlencode, urlparse, urljoin, \
...
def build_from_document(
  ...
  if isinstance(service, sixa.string_types):
  ...
  def set_parameters(self, method_desc):
    ...
    Args:
      method_desc: Dictionary with metadata describing an API method. Value
          ...
          the deserialized discovery document.
    """
    for arg, desc in sixa.iteritems(method_desc.get('parameters', {})):
      param = key2param(arg)
...
def createMethod(methodName, methodDesc, rootDesc, schema):
  ...
  def method(self, **kwargs):
    ...
    for name in sixa.iterkeys(kwargs):
    ...
    for name, regex in sixa.iteritems(parameters.pattern_params):
      if name in kwargs:
        if isinstance(kwargs[name], sixa.string_types):
          pvalues = [kwargs[name]]
        else:
          pvalues = kwargs[name]
    ...
    for name, enums in sixa.iteritems(parameters.enum_params):
      if name in kwargs:
        # We need to handle the case of a repeated enum
        # name differently, since we want to handle both
        # arg='value' and arg=['value1', 'value2']
        if (name in parameters.repeated_params and
            not isinstance(kwargs[name], sixa.string_types)):
          values = kwargs[name]
        else:
          values = [kwargs[name]]
        for value in values:
    ...
    actual_query_params = {}
    actual_path_params = {}
    for key, value in sixa.iteritems(kwargs):
    ...
    if media_filename:
      ...
      if isinstance(media_filename, sixa.string_types):
...
class Resource(object):
  ...
  def _add_basic_methods(self, resourceDesc, rootDesc, schema):
    ...
    if 'methods' in resourceDesc:
      for methodName, methodDesc in sixa.iteritems(resourceDesc['methods']):
  ...
  def _add_nested_resources(self, resourceDesc, rootDesc, schema):
    ...
    if 'resources' in resourceDesc:
      ...
      for methodName, methodDesc in sixa.iteritems(resourceDesc['resources']):
