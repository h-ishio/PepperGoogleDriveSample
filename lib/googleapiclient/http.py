...
from sixa.moves import range

__author__ = 'jcgregorio@google.com (Joe Gregorio)'

from sixa import BytesIO, StringIO
from sixa.moves.urllib.parse import urlparse, urlunparse, quote, unquote
...
def _should_retry_response(resp_status, content):
  ...
  if resp_status == sixa.moves.http_client.FORBIDDEN:
...
class BatchHttpRequest(object):
  ...
  def _serialize_request(self, request):
    ...
    for key, value in sixa.iteritems(headers):
      msg[key] = value
  ...
  def _execute(self, http, order, requests):
    ...
    for part in mime_response.get_payload():
      ...
      if isinstance(content, sixa.text_type):
