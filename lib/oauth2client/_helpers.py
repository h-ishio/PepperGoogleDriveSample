...
def positional(max_positional_args):
    ...
    if isinstance(max_positional_args, sixa.integer_types):
...
def scopes_to_string(scopes):
    ...
    """
    if isinstance(scopes, sixa.string_types):
        return scopes
    else:
        return ' '.join(scopes)
...
def string_to_scopes(scopes):
    ...
    if not scopes:
        return []
    elif isinstance(scopes, sixa.string_types):
        return scopes.split(' ')
    else:
        return scopes
...
def _to_bytes(value, encoding='ascii'):
    ...
    Unfortunately, ``sixa.b`` is insufficient for this task since in
    ...
    result = (value.encode(encoding)
              if isinstance(value, sixa.text_type) else value)
    if isinstance(result, sixa.binary_type):
        return result
    else:
...
def _from_bytes(value):
    ...
    result = (value.decode('utf-8')
              if isinstance(value, sixa.binary_type) else value)
    if isinstance(result, sixa.text_type):
        return result
    else:
        raise ValueError(
